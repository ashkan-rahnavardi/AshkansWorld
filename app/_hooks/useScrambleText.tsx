...
const useTextScramble = (texts: Readonly<string[]>, { letterSpeed = 5, nextLetterSpeed = 100, paused = false, pauseTime = 1500 }: TextScrambleOptions = {}) => {
    const [currentText, setCurrentText] = useState<string>(texts[0]);
    const [scrambledText, setScrambledText] = useState<string[]>(Array(currentText.length).fill(''));

    useEffect(() => {
        let symbolUpdateInterval: NodeJS.Timeout;
        let textUpdateInterval: NodeJS.Timeout;
        let remainingIndexes: number[] = Array.from(Array(currentText.length).keys());
